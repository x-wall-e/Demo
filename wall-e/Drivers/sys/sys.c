...
3.输入参数：
	PLLMUL: PLL倍频系数
4.备注：
	1). 官方手册上说，使用HSI时，最高到达到36M，实际可以达到52M
****************************************************************/
...
3.输入参数：
	PLLMUL: PLL倍频系数
4.备注：
	1). 官方手册上说，使用HSE作为系统时钟源时，最高可倍频到72MHz，但是实际可以倍频到128M系统还算稳定
	2). 时钟源为外部晶振
	3). 当焊接了8M晶振时，就只能使用外部8M晶振作为时钟源
	4). PLL倍频系数，从2开始，最大值为16
****************************************************************************************************/
char SystemClock_HSE(u8 PLL)
{
    ...
    RCC->CFGR|=1<<16;		/* PLL输入时钟源: HSE时钟作为PLL输入时钟 */
